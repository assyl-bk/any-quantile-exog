...
class AnyQuantileForecasterExog(pl.LightningModule):
    ...
    def shared_forward(self, x):
        ...
        x_max = torch.abs(history).max(dim=-1, keepdims=True)[0]
        if self.cfg.model.max_norm:
            x_max = torch.clamp(x_max, min=1.0)  # safe: never div by < 1
        else:
            x_max = torch.ones_like(x_max)
        history = history / x_max
        history = torch.nan_to_num(history, nan=0.0, posinf=1.0, neginf=-1.0)
        ...
        # Pass exog features only when present
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Filter rows where target contains inf or nan (e.g. Iceland series)
        valid = torch.isfinite(target).all(dim=-1)   # [B]
        if valid.any():
            y_hat_point = y_hat[..., 0].contiguous()
            self.val_mse(y_hat_point[valid], target[valid])
            self.val_mae(y_hat_point[valid], target[valid])
            self.val_smape(y_hat_point[valid], target[valid])
            self.val_mape(y_hat_point[valid], target[valid])

        # CRPS and coverage handle invalid targets internally
        self.val_crps(y_hat, target, q=quantiles)
        self.val_coverage(y_hat, target, q=quantiles)
    ...
    def test_step(self, batch, batch_idx):
        ...
        median_idx  = 0  # quantile 0.5 is index 0
        y_hat_point = y_hat[..., median_idx].contiguous()  # [B, H]

        # Filter rows where target contains inf or nan (e.g. Iceland series)
        valid = torch.isfinite(target).all(dim=-1)   # [B]
        if valid.any():
            self.test_mse(y_hat_point[valid], target[valid])
            self.test_mae(y_hat_point[valid], target[valid])
            self.test_smape(y_hat_point[valid], target[valid])
            self.test_mape(y_hat_point[valid], target[valid])

        # CRPS and coverage handle invalid targets internally
        self.test_crps(y_hat, target, q=quantiles)
        self.test_coverage(y_hat, target, q=quantiles)
...
class AQNBEATSPlusPlus(AnyQuantileForecasterExog):
    """
    Extends AnyQuantileForecasterExog with adaptive quantile sampling (Stage 2).
    Inherits all methods — only __init__ and training_step are overridden.

    _bin_probs_np is a NumPy float64 array stored as a plain Python attribute.
    NumPy arrays are completely invisible to Lightning/AMP/CUDA so they can
    never be cast to fp16, solving the multinomial zero-sum assert.
    """

    def __init__(self, cfg):
        super().__init__(cfg)
        adp_cfg = getattr(cfg.model, 'adaptive_sampling', None)
        n_bins = adp_cfg.num_bins if adp_cfg is not None else 30
        # NumPy float64 — Lightning/AMP cannot touch this
        self._bin_probs_np = np.ones(n_bins, dtype=np.float64) / n_bins
    ...
    def training_step(self, batch, batch_idx):
        ...
        elif self.cfg.model.q_sampling == 'adaptive':
            adp     = self.cfg.model.adaptive_sampling
            n_bins  = adp.num_bins
            n_adapt = adp.num_adaptive_quantiles

            q_beta = torch.Tensor(
                np.random.beta(
                    self.cfg.model.q_parameter,
                    self.cfg.model.q_parameter,
                    size=(batch_size - n_adapt, 1)
                )
            ).to(batch['history'])

            # Convert numpy → fresh fp32 tensor each step
            # This is the only safe pattern under fp16 AMP
            with torch.no_grad():
                probs   = torch.tensor(self._bin_probs_np, dtype=torch.float32,
                                       device=self.device)
                probs   = torch.clamp(probs, min=1e-6)
                probs   = probs / probs.sum()
                bin_idx = torch.multinomial(probs, n_adapt, replacement=True)
                bin_lo  = bin_idx.float() / n_bins
                bin_hi  = (bin_idx.float() + 1) / n_bins
                q_adapt = (bin_lo + torch.rand(n_adapt, device=self.device)
                           * (bin_hi - bin_lo)).unsqueeze(1)

            batch['quantiles'] = torch.cat([q_beta, q_adapt], dim=0).clamp(1e-6, 1 - 1e-6)
        ...
        # ── EMA update of bin_probs — stored back to numpy ────────────────────
        if self.cfg.model.q_sampling == 'adaptive':
            with torch.no_grad():
                adp    = self.cfg.model.adaptive_sampling
                n_bins = adp.num_bins

                q_flat   = batch['quantiles'].squeeze(1).float()
                y_hat_1q = y_hat[..., center_idx].float()
                resid    = batch['target'].float() - y_hat_1q
                pb = torch.where(
                    resid >= 0,
                    q_flat[:, None] * resid,
                    (q_flat[:, None] - 1) * resid
                ).mean(dim=-1)
                # Use absolute loss as difficulty signal (always non-negative)
                pb = torch.abs(pb)

                bin_idx   = (q_flat * n_bins).long().clamp(0, n_bins - 1)
                new_probs = torch.zeros(n_bins, device=self.device, dtype=torch.float32)
                counts    = torch.zeros(n_bins, device=self.device, dtype=torch.float32)
                new_probs.scatter_add_(0, bin_idx, pb.float())
                counts.scatter_add_(0, bin_idx, torch.ones_like(pb, dtype=torch.float32))
                mask = counts > 0
                new_probs[mask] = new_probs[mask] / counts[mask]
                # Clamp BEFORE exp to prevent underflow from negative values
                new_probs = torch.clamp(new_probs, min=1e-6, max=1e2)
                new_probs = torch.exp(new_probs * adp.temperature)
                new_probs = torch.clamp(new_probs, min=adp.min_prob, max=1e6)
                new_probs = new_probs / new_probs.sum()

                # EMA entirely in numpy — safe from AMP
                np_new = new_probs.cpu().numpy().astype(np.float64)
                self._bin_probs_np = (adp.momentum * self._bin_probs_np
                                      + (1 - adp.momentum) * np_new)
                self._bin_probs_np = np.clip(self._bin_probs_np, 1e-6, 1e6)
                self._bin_probs_np = self._bin_probs_np / self._bin_probs_np.sum()
